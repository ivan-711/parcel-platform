...
export function useSkipTrace() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['u', userId, 'skip-tracing'] })
...
export function useSkipTraceHistory(filters?: SkipTraceHistoryFilters) {
  const userId = useAuthStore((s) => s.user?.id)
  return useQuery({
    queryKey: ['u', userId, 'skip-tracing', 'history', filters],
...
export function useSkipTraceDetail(id: string | undefined) {
  const userId = useAuthStore((s) => s.user?.id)
  return useQuery({
    queryKey: ['u', userId, 'skip-tracing', id],
...
export function useSkipTraceUsage() {
  const userId = useAuthStore((s) => s.user?.id)
  return useQuery({
    queryKey: ['u', userId, 'skip-tracing', 'usage'],
...
export function useBatchStatus(batchId: string | undefined) {
  const userId = useAuthStore((s) => s.user?.id)
  return useQuery({
    queryKey: ['u', userId, 'skip-tracing', 'batch', batchId],
