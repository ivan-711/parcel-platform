...
class AnimationController {
    ...
    private readonly cameraZ = -300
    private readonly cameraTravelDistance = 4500
    private readonly startDotYOffset = 28
    private readonly viewZoom = 160
    private readonly numberOfStars = 6000
    private readonly trailLength = 120
    ...
    public showProjectedDot(position: Vector3D, sizeFactor: number) {
        ...
        if (position.z > newCameraZ) {
            ...
            this.ctx.beginPath()
            this.ctx.arc(x, y, Math.max(sw * 0.06, 0.6), 0, Math.PI * 2)
            this.ctx.fill()
        }
    }
    ...
    public render() {
        ...
        this.drawSpiral(this.time, true)
    }

    private drawSpiral(adjustedTime: number, mirror: boolean) {
        const ctx = this.ctx
        ...
        const t1 = this.constrain(this.map(adjustedTime, 0, this.changeEventTime + 0.25, 0, 1), 0, 1)
        const t2 = this.constrain(this.map(adjustedTime, this.changeEventTime, 1, 0, 1), 0, 1)
    ...
    private drawTrail(t1: number) {
        for (let i = 0; i < this.trailLength; i++) {
            ...
            const trailScale = this.isLight ? 1.3 : 1.0
            const sw = (3.0 * (1 - t1) + 7.0 * Math.sin(Math.PI * t1)) * f * trailScale
            this.ctx.fillStyle = '#8B7AFF'
            this.ctx.lineWidth = sw
...
class Star {
    ...
    private isViolet: boolean

    constructor(cameraZ: number, cameraTravelDistance: number) {
        this.angle = Math.random() * Math.PI * 2
        this.distance = 100 * Math.random() + 60
        this.rotationDirection = Math.random() > 0.5 ? 1 : -1
        this.expansionRate = 1.2 + Math.random() * 0.8
        this.finalScale = 0.7 + Math.random() * 0.6
        this.isViolet = Math.random() < 0.1
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            const powerEasing = Math.pow(displacementProgress, 1.3)
            let easing
            ...
            const violetMul = this.isViolet ? 1.3 : 1.0
            const dotSize = 24 * this.strokeWeightFactor * sizeMultiplier * violetMul
            const distFromCenter = Math.sqrt(screenX * screenX + screenY * screenY)
            const opacityFade = Math.max(controller.isLight ? 0.8 : 0.3, 1 - (distFromCenter / (controller.isLight ? 350 : 300)))
            controller.ctx.globalAlpha = opacityFade
            // Violet particles stay violet in both themes; cream particles swap to warm dark gray in light mode
            controller.ctx.fillStyle = this.isViolet ? '#8B7AFF' : (controller.isLight ? '#0C0B0A' : '#F0EDE8')
            controller.showProjectedDot(position, dotSize)
            controller.ctx.globalAlpha = 1.0
        }
    }
}
...
export function SpiralBackground() {
    ...
    // Sync isLight to the animation controller without recreating it
    useEffect(() => {
        if (animationRef.current) {
            animationRef.current.isLight = isLight
        }
    }, [isLight])
    ...
    useEffect(() => {
        ...
        ctx.scale(dpr, dpr)
        animationRef.current = new AnimationController(canvas, ctx, dpr, size, isLight)
        return () => {
            if (animationRef.current) {
