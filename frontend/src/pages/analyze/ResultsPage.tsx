...
export default function ResultsPage() {
  ...
  const handleExportPDF = () => {
    ...
    const strat = deal.strategy
    const risk = deal.risk_score ?? 0

    // Header
    doc.setFontSize(20)
    doc.setTextColor(99, 102, 241)
    doc.text('PARCEL', 15, 20)
    doc.setTextColor(0, 0, 0)
    doc.setFontSize(12)
    doc.text('Deal Analysis Report', 15, 30)
    doc.setFontSize(10)
    ...
    doc.text(addr, 15, 52)
    doc.setFontSize(11)
    doc.text(`Strategy: ${strat.replace(/_/g, ' ').toUpperCase()}`, 15, 62)
    doc.text(`Risk Score: ${risk}/100`, 15, 70)

    // Outputs
    doc.setFontSize(13)
    doc.text('Analysis Outputs', 15, 84)
    doc.setFontSize(10)
    let y = 94
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text('Generated by Parcel · parcel-platform-kappa.vercel.app', 15, 290)
      doc.text(`Page ${i} of ${pageCount}`, 180, 290)
    }

    const filename = `${addr.replace(/[^a-z0-9]/gi, '-').toLowerCase()}-analysis.pdf`
    doc.save(filename)
  }
